...
14. 🟡 DLD / DDA API integration — credentials live, blocked by UAE-only geo policy
    ...
    - ✅ **Endpoint paths corrected** 1 May 2026 — token URL is `/secure/ssis/dubaiai/gatewaytoken/1.0.0/getAccessToken` (was `/secure/sdg/ssis/gatewayoauthtoken/...`, a guess from the v1 generic spec). Data path unchanged: `/secure/ddads/openapi/1.0.0/<entity>/<dataset>`
    - 🟡 **HARD GEO-BLOCK confirmed** in HowToUseAPI.pdf §6.3: *"All the APIs can be accessed over the internet, within the United Arab Emirates. The APIs are inaccessible outside the country."* Our Supabase Edge Functions egress from Tokyo (`ap-northeast-1`) and Vercel from US — both 403'd. This is policy, not allowlist; no email can fix it
    - ⬜ **Architecture decision pending** — UAE-resident relay required. Three options on the table:
        1. **AWS Lambda in `me-central-1`** (UAE region) — exposed via Function URL, called from Supabase `dld-proxy` as a tunnel. Free tier covers 1M req/month
        2. **Azure Functions UAE North** — same idea, Azure equivalent
        3. **Cheap UAE VPS** (~$5–10/mo, e.g. ServerGenie / dxbservers) — runs a tiny Node/Deno relay
        Recommendation: AWS Lambda — zero infra to maintain, fits inside free tier
    - ⬜ **Founder actions before relay deploy:**
        1. Update Supabase Edge Function secrets in dashboard with the new credentials from the 1 May email — secret names: `DDA_APP_IDENTIFIER`, `DDA_CLIENT_ID`, `DDA_CLIENT_SECRET`. Set `DDA_BASE_URL=https://stg-apis.data.dubai`. Leave `DDA_ENABLED=false` until relay is live
        2. **Smoke-test from a Dubai IP** (founder's home/office laptop) using the curl in HowToUseAPI.pdf §3.2 — confirms credentials work end-to-end before we invest in relay infra
        3. **Request specific dataset access on data.dubai portal** for DLD transactions (per HowToUseAPI.pdf §6.5: "Request API Access Key" button on each dataset's page is a one-time per-dataset action). The credentials work, but data calls 401 until the specific dataset is granted
    - ⬜ **Once relay is deployed + dataset granted:** flip `DDA_ENABLED=true`, set `DDA_UAE_RELAY_URL=<lambda-fn-url>`, smoke-test against the granted dataset, update the entity allow-list inside `dld-proxy/index.ts` to match. Production keys come in a separate email from DDADS after STG validation
