...
14. 🟡 DLD / DDA API integration — wired, awaiting IP allowlist
    - ✅ **DDA Open/Shared API credentials received** 27 Apr 2026 — application `PUBLIC-USR-UID-4511215`, **STG environment** (production keys come after STG validation, per DDA process)
    - ✅ **3 reference docs decoded** (`docs/ICD_GSB-SSIS-002-…GatewayToken_v1.0.0_Generic.docx`, `docs/SDG-DDADS-HealthCheck_PUBLICUSER_v2.docx`, `docs/SDG-DDADS-OpenAPI_PUBLICUSER_v2.1.docx`). Two-step flow: OAuth client_credentials → Bearer token (3600s TTL) → GET `/secure/ddads/openapi/1.0.0/<entity>/<dataset>` with `x-DDA-SecurityApplicationIdentifier` header
    - ✅ **5 DDA secrets saved to Supabase Edge Function vault** (`DDA_CLIENT_ID`, `DDA_CLIENT_SECRET`, `DDA_APP_IDENTIFIER`, `DDA_BASE_URL`, `DDA_ENABLED=false`). Secrets are read at request-time so flipping the flag in the Supabase dashboard requires no redeploy
    - ✅ **`dld-proxy` rewritten + deployed** (v1, ACTIVE, `verify_jwt: true`): handles OAuth flow, caches token in module-scope (refreshes ~1 min before expiry), enforces entity allow-list (`dld`, `det`, `rta`, `dsc`), forwards the 8 supported DDA query params, 30s timeout per DDA spec, returns `{ fallback: true, source: 'cache' }` (HTTP 503) when disabled or auth fails — same shape as the previous shell so existing UI consumers keep working unchanged
    - 🟡 **DDA gateway returns HTTP 403 "Unauthorized application request"** on getAccessToken from our egress IP — confirmed via direct probe with credentials. The gateway recognises the application ID, so this is **IP allowlisting on DDA's side**, not a credential issue
    - 🟡 **DDA STG SSL certificate is currently expired** (cosmetic, but worth flagging back to DDADS)
    - ⬜ **Founder action — reply to DDADS support** (`DDADS.support@digitaldubai.ae`) requesting: (1) IP allowlist for our Supabase Edge Function egress region (`ap-northeast-1` / Tokyo) and Vercel serverless egress; (2) confirmation of the granted `<entity>/<dataset>` list — the email's "attached APIs" are generic spec docs, not a granted-datasets list; (3) heads-up on the expired STG SSL cert
    - ⬜ **Once DDADS confirms allowlist + datasets:** flip `DDA_ENABLED=true` in Supabase secrets, smoke-test against one known dataset, then update the dataset allow-list inside `dld-proxy/index.ts` to match the granted list. Production keys come from DDA in a separate email after STG validation period
